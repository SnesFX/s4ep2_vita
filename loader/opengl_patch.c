...
void patch_opengl(void) {
  *(char **)so_symbol(&s4ep2_mod, "shadowResolvePShader") = shadowResolvePShader;
  *(char **)so_symbol(&s4ep2_mod, "blurPShader") = blurPShader;
  *(char **)so_symbol(&s4ep2_mod, "gradingPShader") = gradingPShader;
  *(char **)so_symbol(&s4ep2_mod, "contrastPShader") = contrastPShader;
  *(char **)so_symbol(&s4ep2_mod, "contrastVShader") = contrastVShader;

  GetMobileEffectSetting = (void *)so_symbol(&s4ep2_mod, "_Z22GetMobileEffectSettingv");

  RQCaps = (RQCapabilities *)so_symbol(&s4ep2_mod, "RQCaps");
  RQMaxBones = (int *)so_symbol(&s4ep2_mod, "RQMaxBones");
  hook_addr(so_symbol(&s4ep2_mod, "_ZN8RQShader11BuildSourceEjPPKcS2_"), (uintptr_t)RQShader__BuildSource);
}
