...
__attribute__((naked)) void ColorFilter_stub(void) {
	...
	register uintptr_t retAddr asm ("r12") = (uintptr_t)s4ep2_mod.text_base + 0x005B6444 + 0x1;
...
patch_gfx(void)
{
	p_pAmbient = (RpLight**)so_symbol(&s4ep2_mod, "pAmbient");
	p_pDirect = (RpLight**)so_symbol(&s4ep2_mod, "pDirect");
	p_AmbientLightColourForFrame = (RwRGBAReal*)so_symbol(&s4ep2_mod, "AmbientLightColourForFrame");
	p_AmbientLightColourForFrame_PedsCarsAndObjects = (RwRGBAReal*)so_symbol(&s4ep2_mod, "AmbientLightColourForFrame_PedsCarsAndObjects");
	p_DirectionalLightColourForFrame = (RwRGBAReal*)so_symbol(&s4ep2_mod, "DirectionalLightColourForFrame");
	p_DirectionalLightColourFromDay = (RwRGBAReal*)so_symbol(&s4ep2_mod, "DirectionalLightColourFromDay");
	p_CTimeCycle__m_CurrentColours = (CColourSet*)so_symbol(&s4ep2_mod, "_ZN10CTimeCycle16m_CurrentColoursE");
	p_CTimeCycle__m_vecDirnLightToSun = (CVector*)so_symbol(&s4ep2_mod, "_ZN10CTimeCycle19m_vecDirnLightToSunE");
	p_gfLaRiotsLightMult = (float*)so_symbol(&s4ep2_mod, "gfLaRiotsLightMult");
	p_CCoronas__LightsMult = (float*)so_symbol(&s4ep2_mod, "_ZN8CCoronas10LightsMultE");
	p_CWeather__LightningFlash = (uint8_t*)so_symbol(&s4ep2_mod, "_ZN8CWeather14LightningFlashE");

	RwFrameTransform = (RwFrame *(*)(RwFrame*,const RwMatrix*,RwOpCombineType))so_symbol(&s4ep2_mod, "_Z16RwFrameTransformP7RwFramePK11RwMatrixTag15RwOpCombineType");
	RpLightSetColor = (RpLight *(*)(RpLight*, const RwRGBAReal*))so_symbol(&s4ep2_mod, "_Z15RpLightSetColorP7RpLightPK10RwRGBAReal");

	openglAmbientLight = (float*)so_symbol(&s4ep2_mod, "openglAmbientLight");
	p_rwOpenGLColorMaterialEnabled = (RwInt32*)so_symbol(&s4ep2_mod, "_rwOpenGLColorMaterialEnabled");

	emu_glLightModelfv = (void (*)(GLenum, const GLfloat *))so_symbol(&s4ep2_mod, "_Z18emu_glLightModelfvjPKf");
	emu_glMaterialfv = (void (*)(GLenum, GLenum, const GLfloat *))so_symbol(&s4ep2_mod, "_Z16emu_glMaterialfvjjPKf");
	emu_glColorMaterial = (void (*)(GLenum, GLenum))so_symbol(&s4ep2_mod, "_Z19emu_glColorMaterialjj");	// no-op
	emu_glEnable = (void (*)(GLenum))so_symbol(&s4ep2_mod, "_Z12emu_glEnablej");
	emu_glDisable = (void (*)(GLenum))so_symbol(&s4ep2_mod, "_Z13emu_glDisablej");

	if(config.skygfx_ps2_shading){
		// upload all material data regardless of shader flags
		const uint16_t nop = 0xbf00;
		kuKernelCpuUnrestrictedMemcpy((void *)(s4ep2_mod.text_base + 0x1C1382), &nop, sizeof(nop));
		kuKernelCpuUnrestrictedMemcpy((void *)(s4ep2_mod.text_base + 0x1C13BA), &nop, sizeof(nop));
		hook_addr(so_symbol(&s4ep2_mod, "_Z36_rwOpenGLLightsSetMaterialPropertiesPK10RpMaterialj"), (uintptr_t)_rwOpenGLLightsSetMaterialProperties);

		hook_addr(so_symbol(&s4ep2_mod, "_Z28SetLightsWithTimeOfDayColourP7RpWorld"), (uintptr_t)SetLightsWithTimeOfDayColour);
	}
	...
	if(config.skygfx_colorfilter != SKYGFX_COLOR_FILTER_MOBILE){
		...
		hook_addr((uintptr_t)s4ep2_mod.text_base + 0x005B643C + 0x1, (uintptr_t)ColorFilter_stub);
		kuKernelCpuUnrestrictedMemcpy((void *)(s4ep2_mod.text_base + 0x005B6444), (void *)(s4ep2_mod.text_base + 0x005B63DC), sizeof(uint16_t));
		kuKernelCpuUnrestrictedMemcpy((void *)(s4ep2_mod.text_base + 0x005B6446), (void *)(s4ep2_mod.text_base + 0x005B63EA), sizeof(uint16_t));
	}

	// Enable PS2-like sun corona
	if(config.skygfx_ps2_sun){
		const uint32_t nop2 = 0xbf00bf00;
		kuKernelCpuUnrestrictedMemcpy((void *)(s4ep2_mod.text_base + 0x005A26B0), &nop2, sizeof(nop2));
	}
}
